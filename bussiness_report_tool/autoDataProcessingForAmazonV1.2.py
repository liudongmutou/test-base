...
#--------1.1 当有多个文件夹下数据需要处理时，可通过此命令选择待处理文件夹

processingDir=input("please enter your target folder: ")
print('-----***-----\n')

theYear=input("please input the Year: ")
theMonth=input("please input the Month: ")
theWeek=input("please input the Week: ")
print('-----***-----\n')
#----------- 1.1.1 获得待处理的目标文件目录
...
mkdir(filesDir+'./outputDir')        
outputDir=filesDir+'./outputDir./'
...
for i in filesArray:
    ...
    if iFormat == 'csv':
        ...
        otherfileName=os.path.splitext(i)[0]
        otherInput=i
...
def txt2xlsx(filename,xlsxname):
    try:
...
txt2xlsx(filesDir+'/'+txtInput,outPutFile)
print('txt文件格式转换完毕')
...
if  src_wb['业务报告'].max_row < dst_wb['综合报告'].max_row:
    for row in range(dst_head+1, dst_wb['业务报告'].max_row+1):
        dst_head=dst_head+1
        dst_wb['综合报告'].cell(row=dst_head, column=1).value =''
